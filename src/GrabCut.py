...
#        mask_fore[:,:,0][white > 205] = 255
#        mask_fore[:,:,2][white > 205] = 255
#        mask_back[:,:,0][black == 1] = 0
#        mask_back[:,:,1][black == 1] = 138
#        mask_back[:,:,2][black == 1] = 162
#        mask_back[:270,-20:,0] = 0
#        mask_back[:270,-20:,1] = 138
#        mask_back[:270,-20:,2] = 162
#        mask_back[:270,:20,0] = 0
#        mask_back[:270,:20,1] = 138
#        mask_back[:270,:20,2] = 162
#
...
#        mask_back[:5,:,2] = 162
        ...
        mask_global = np.where((mask_global==2)|(mask_global==0),0,1).astype('uint8')
        target = img*mask_global[:,:,np.newaxis]
        #target[mask_back>0] = mask_back[mask_back>0]
        cv2.imshow('target',target)
        valid, masked = merge(mask_fore, mask_back, img)
        cv2.imwrite('target.bmp',masked)
        
        cv2.imwrite('result.bmp',target)
